...
class HistoryPreprocessor(Preprocessor):
    ...
    def __init__(self, history_length=1, policy_length = 15):
        self.history_length = history_length
        self.policy_length = policy_length
        self.past_states = None 
        self.past_states_ori = None
        self.past_policy =  np.zeros((self.policy_length, 2))
    ...
    def process_policy_for_network(self, policy):
        # print("policy size", policy.shape, self.past_policy.shape)
        history = np.concatenate((self.past_policy, policy), axis = 0)
        # print("policy size", policy.shape, self.past_policy.shape)
        self.past_policy = history[1:, :]
        return history
