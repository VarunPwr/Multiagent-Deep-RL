...
class Sample:
    ...
    """
    def __init__(self, state, action, reward, next_state, is_terminal, q_value=None, next_q_value=None, weight = None, index = None):
        self.state = state
        self.action = action
        self.reward = reward
...
class ReplayMemory:
    ...
    """
    def __init__(self, args, num_actions=2):
        """Setup memory.
        ...
        self.policy_length = args.num_policies
        ...
        self.q_values = np.zeros((self.memory_size, self.num_actions), dtype = np.float32 )

        self.current = 0

    def append(self, state, action, reward, is_terminal, q_value=None):
        ...
        self.q_values[self.current % self.memory_size] = q_value
    ...
    def get_policy(self, index):
        policy = self.q_values[index - self.history_length + 1:index + 1, :]
        # history dimention last
        return policy
    ...
    def sample(self, batch_size):
        ...
        while len(indexes) < batch_size: 
            index = np.random.randint(max(self.history_length, self.policy_length) - 1, end)
            # sampled state shouldn't contain episode end
            if self.terminals[index - max(self.history_length, self.policy_length) + 1: index + 1].any():
                continue
            indexes.append(index)

        for idx in indexes:
            new_sample = Sample(self.get_state(idx), self.actions[idx],\
                self.rewards[idx], self.get_state(idx+1), self.terminals[idx], self.get_policy(idx), self.get_policy(idx+1))
