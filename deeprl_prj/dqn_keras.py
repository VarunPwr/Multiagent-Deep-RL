...
def create_model(input_shape, num_actions, mode, args, model_name='q_network'):  # noqa: D103
    ...
    with tf.compat.v1.variable_scope(model_name):
        ...
        # Select either a linear or dueling linear layer.
        if mode == "dqn":
...
class DQNAgent:
    ...
    def update_policy(self, current_sample):
        ...
        with tf.GradientTape() as tape:
            qa_value = self.q_network(states)
            qa_value = Multiply(name = "multiply")([qa_value, action_mask])
            qa_value = Lambda(lambda x: tf.reduce_sum(x, axis=1, keepdims = True), name = "sum")(qa_value)
            loss_value = self.loss_fn(target, tf.squeeze(qa_value, axis= 1))
        grads = tape.gradient(loss_value, self.final_model.trainable_weights)
        self.optimizer.apply_gradients(zip(grads, self.final_model.trainable_weights))

        return loss_value.numpy().mean() , np.mean(target)
